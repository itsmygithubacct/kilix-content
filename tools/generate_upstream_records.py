...
def _pin_match(path: Path, name: str) -> tuple[str, re.Match[str]]:
...
def _bindings(tree: ast.Module, name: str) -> list[ast.AST]:
    """Every node in the module that binds or unbinds name."""
    found: list[ast.AST] = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Name):
            bound = node.id if not isinstance(node.ctx, ast.Load) else None
            ...
            bound = None
        if bound == name or bound == "*":
            found.append(node)
    return found


def check_pin(path: Path, name: str, expected: str) -> None:
    """--check: a hand-edited pin fails like a hand-edited catalog (C2E-VERIFY F7).

    The runtime uses the last binding of the name, so exactly one binding is
    allowed: a module-level string assignment (C2E-FIX-VERIFY R5).
    """
    text, match = _pin_match(path, name)
    relative = path.relative_to(ROOT)
    tree = ast.parse(text, filename=str(relative))
    bindings = _bindings(tree, name)
    if len(bindings) != 1:
        lines = ", ".join(str(getattr(node, "lineno", "?")) for node in bindings)
        raise SystemExit(
            f"{name} is bound {len(bindings)} times in {relative} (lines {lines}); "
            "make pins allows exactly one assignment"
        )
    assignment = next(
        (
            node
    ...
    if assignment is None:
        raise SystemExit(f"{name} in {relative} is not a module-level string assignment")
    value = assignment.value.value
    if value != match.group(2):
        raise SystemExit(f"{name} in {relative} is {value}, not the pin text {match.group(2)}")
    if value != expected:
        raise SystemExit(
            f"{name} in {relative} is {value}, "
            f"but the generated bytes hash to {expected}"
        )
...
def main(argv: list[str]) -> int:
    ...
    if args.check:
        ...
        check_pin(RECEIPT, "_CATALOG_SHA256", catalog_sha)
        check_pin(RECEIPT, "_ASSET_V3_SCHEMA_SHA256", schema_sha)
        return 0
