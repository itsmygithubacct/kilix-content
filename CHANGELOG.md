...
- Re-pin kilix-needle to `5a1acada` (0.2.2 reviews R2 to R5). Every action in a request is resolved
  ...
  person who sees the resolved target. A yes given in advance also never
  closes the requester's own pane or tab, and never acts on a name found
  only as a word of a title; a pane is read again before anything is typed
  into it.
